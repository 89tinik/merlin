...
if(is_login()){
    ...
    if (is_admin($_SESSION['user']) || is_author($_POST['user_id'])){
       if( edit_credentianals($_POST['user_id'], $_POST['email'], $_POST['password'])){
           set_flash_message('Данные успешно изменены.', 'success');
           redirect_to('users.php');
       } else {
           set_flash_message('Каких-то данных не хватает.', 'danger');
           redirect_to('security.php?id='.$_POST['user_id']);
       }
    } else {
